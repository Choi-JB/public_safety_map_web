...
import type { ChatMessage, RoomAccessInfo } from "@/lib/chat/types";
...
export function ChatPanel({ roomId }: ChatPanelProps) {
  ...
  const [roomAccess, setRoomAccess] = useState<RoomAccessInfo | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const access = await getRoomAccessInfo(roomId, myUserId, userRole);
        if (cancelled) return;
        setRoomAccess(access);

        if (!access.canAccess) {
          setError("이 방에 접근할 수 없습니다.");
          setStatus("접근 불가");
          return;
        }

        const past = await fetchMessages(roomId, myUserId, userRole);
        if (cancelled) return;
        setMessages(past);
  ...
  }, [roomId, isLoggedIn, nickname, myUserId, userRole]);
